...
void Stack_Init( Stack *stack ) {
    ...
    stack->array=malloc(sizeof(char)*STACK_SIZE); //allocate memory for stack array
    stack->topIndex=-1;                             //initialize topIndex to -1(symboling empty stack)
    return;
...
bool Stack_IsEmpty( const Stack *stack ) {
    return (stack->topIndex==-1)?true:false;    //ternary operator for checking if stack is empty
	//solved = false; /* V případě řešení, smažte tento řádek! */
}
...
bool Stack_IsFull( const Stack *stack ) {
    return (stack->topIndex==STACK_SIZE-1)?true:false;    //ternary operator for checking if stack is full
	//solved = false; /* V případě řešení, smažte tento řádek! */
}
...
void Stack_Top( const Stack *stack, char *dataPtr ) {
    if (Stack_IsEmpty(stack)) {     //check if stack is empty
        Stack_Error(3);
    }
    else {
        *dataPtr=stack->array[stack->topIndex]; //return top element
        return;
    }
	//solved = false; /* V případě řešení, smažte tento řádek! */
}
...
void Stack_Pop( Stack *stack ) {
    if (!Stack_IsEmpty(stack)) { //check if stack is not empty
        ...
        stack->topIndex--; //decrement top index
        return;
    }
	//solved = false; /* V případě řešení, smažte tento řádek! */
}
...
void Stack_Push( Stack *stack, char data ) {
    if (!Stack_IsFull(stack)) { //check if stack is not full
        stack->array[++stack->topIndex]=data; //increment top index and assign data
    }
    else{
        Stack_Error(2);
    }
...
void Stack_Dispose( Stack *stack ) {
    if(stack->array!=NULL){ //check if array is not null
        free(stack->array); //free array memory
        stack->array=NULL;  //set array pointer to null
    }
    stack->topIndex=-1; //reset top index to -1
    return;
	//solved = false; /* V případě řešení, smažte tento řádek! */
}
